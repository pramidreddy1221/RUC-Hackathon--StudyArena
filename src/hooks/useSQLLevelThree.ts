...
interface UseSQLLevelThreeProps {
  questions: SQLLevelThreeItem[]
  onComplete: () => void
}

export function useSQLLevelThree({ questions, onComplete }: UseSQLLevelThreeProps) {
  ...
  const submitAnswer = useCallback(() => {
    ...
    if (normalizedUserAnswer === normalizedCorrectAnswer) {
      setIsCorrect(true)
      setIsLocked(true)
      setFeedbackMessage('Perfect! Your complex SQL query is correct.')
    ...
    // Try pattern-based matching (especially important for complex queries)
    if (sqlMatchesPattern(userAnswer, currentQuestion.answer)) {
      setIsCorrect(true)
      setIsLocked(true)
      setFeedbackMessage('Excellent! Your SQL logic is correct.')
